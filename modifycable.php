...
<body>
    <div class="title"><h1>Modify/View Product</h1></div> 
        ...
        <select class="producttype" name="producttype" onchange="window.location=this.value">
            ...
            <option value="" disabled selected hidden>Cable</option>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            ...
            $query = "SELECT * FROM `product` WHERE `product_name` LIKE '%$searchname%' AND `product_category` = 'Cable'";
            ...
            $query = "SELECT * FROM `product` WHERE `product_category` = 'Cable'";
            $product = $conn->query($query) or die($conn->error);
