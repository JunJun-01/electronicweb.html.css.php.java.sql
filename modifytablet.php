...
<body>
    <div class="title"><h1>Modify/View Product</h1></div> 
        ...
        <select class="producttype" name="producttype" onchange="window.location=this.value">
            ...
            <option value="modifyphone.php">Phone</option>
            <option value="" disabled selected hidden>Tablet</option>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            ...
            $query = "SELECT * FROM `product` WHERE `product_name` LIKE '%$searchname%' AND `product_category` = 'Tablet'";
            ...
            $query = "SELECT * FROM `product` WHERE `product_category` = 'Tablet'";
            $product = $conn->query($query) or die($conn->error);
